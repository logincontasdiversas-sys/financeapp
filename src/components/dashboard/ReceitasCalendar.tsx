...
export const ReceitasCalendar = () => {
  ...
  const loadTransactions = async () => {
    ...
    try {
      ...
      console.log('[RECEITAS CALENDAR] Loading transactions from', startDateStr, 'to', endDateStr);
      ...
      console.log('[RECEITAS CALENDAR] Transactions loaded:', validTransactions.length);
      console.log('[RECEITAS CALENDAR] Sample dates:', validTransactions.slice(0, 5).map(t => t.date));
      
      setTransactions(validTransactions);
    } catch (error) {
      console.error('[RECEITAS CALENDAR] Error loading transactions:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getDatesWithTransactions = () => {
    ...
    console.log('[RECEITAS CALENDAR] Transaction dates for highlighting:', dates.map(d => d.toISOString().split('T')[0]));
    return dates;
  };
    ...
    <Card>
      <CardHeader>
        ...
        <div className="text-xs text-muted-foreground">
          Total de Receitas: {transactions.length}
        </div>
      </CardHeader>
      <CardContent>
        <div className="grid md:grid-cols-2 gap-6">
          <div>
            <Calendar
              ...
              modifiers={{
                hasTransaction: (date) => {
                  ...
                  if (hasTransaction) {
                    console.log('[RECEITAS CALENDAR] Date highlighted:', dateStr, 'transactions:', transactions.filter(t => t.date === dateStr));
                  }
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Dias destacados possuem receitas registradas
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            {selectedDateTransactions.length > 0 ? (
                  ...
                  <div key={transaction.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <Badge variant={transaction.status === 'settled' ? 'default' : 'secondary'}>
                        {transaction.status === 'settled' ? 'Recebido' : 'Pendente'}
                      </Badge>
                    </div>
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>Nenhuma receita registrada para esta data</p>
              </div>
            )}
          </div>
        </div>
