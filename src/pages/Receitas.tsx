            ...
            {(() => {
              ...
              return Object.entries(groupedByDate).map(([date, receitasOfDate]) => (
                <div key={date} className="bg-muted/50 rounded-lg p-4 mb-4">
                    ...
                    <div className="flex-1 relative">
                      <div className="w-full h-1 bg-muted rounded-full relative">
                        ...
                        {receitasOfDate.map((receita, index) => (
                          <div key={receita.id} className="absolute w-4 h-4 rounded-full transform -translate-x-1/2 -translate-y-1/2" 
                               style={{ left: `${(index / (receitasOfDate.length - 1)) * 100}%`, top: '50%' }}>
                  ...
                  <div className="space-y-2">
                    {receitasOfDate.map((receita, index) => (
                      <div key={receita.id} className="flex items-center gap-3 p-3 bg-background rounded-md border">
                        {/* Checkbox */}
                        <Checkbox
